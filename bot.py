...
TRIGGER_WORDS = ["人机", "燕燕生气了", "人呢", "deepseek","DS"] # 敏感词：群里一提到这些，必然跳出来接茬！
...
# WEATHER_REENTRY_GAP_HOURS = 4 # weather 暂时关闭
...
CLAUDE_MODEL_RAW = os.environ.get("CLAUDE_MODEL")
...
#     try:
#         if prev_ts:
#             last = datetime.strptime(prev_ts, "%Y-%m-%d %H:%M:%S").replace(tzinfo=tz)
#             gap_hours = (datetime.now(tz) - last).total_seconds() / 3600
#         else:
#             gap_hours = 1e9
...
def process_message_background(text, chat_id, sender_name, msg_date=None, should_reply=True, msg_id=None,
    ...
    try:
       ...
       # ==========================================
        ...
        # weather：用上一条历史的时间算间隔（必须在 append 当前消息之前取）
        _prev_ts = history[-1].get("timestamp") if history else None
